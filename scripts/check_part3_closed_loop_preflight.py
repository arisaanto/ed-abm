...
import config
from scripts.build_part3_appraisal_packets import build_packets
from scripts.analyze_part3_appraisals import analyze as analyze_appraisals
from scripts.run_part3_closed_loop import (
    VLLMCategoricalCausalProvider,
    _persona_assignment,
    _validate_runtime_contract,
)
from scripts.run_part3_closed_loop_batch import (
    _read_manifest,
    run_parallel_workers,
)
from scripts.plan_part3_synthetic_study import scientific_decision_packet
from scripts.run_single import run_single
from scripts.verify_part3_closed_loop_gate import (
    DECISION_KEYS,
    GROUNDED_MEMORY_FIELDS,
    VISIBLE_MEMORY_FIELDS,
    verify,
)
from scripts.verify_part3_closed_loop_pilot import (
    _sampling_value as aggregate_sampling_value,
    verify as verify_aggregate_pilot,
)
from scripts.verify_part3_vllm_responses import verify as verify_vllm_responses
...
def _module_sources() -> dict[str, str]:
    ...
    import scripts.plan_part3_synthetic_study as planner
    import scripts.run_part3_closed_loop as runner
    import scripts.run_single as single
    import scripts.verify_part3_closed_loop_gate as verifier
...
def _deployed_source_hashes(
    ...
    for module in tuple(sys.modules.values()):
        ...
        if path.suffix == ".py" and PROJECT_DIR in path.parents:
            paths.add(path)
    paths.add(
        (PROJECT_DIR / "jobs/snellius_part3_qwen36_closed_loop_gate.sbatch").resolve()
    )
    for relative in (
