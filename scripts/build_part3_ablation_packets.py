...
from the accepted closed-loop main study and never runs the ABM or an LLM. The
already observed full-persona/full-memory response is the reference cell; three
counterfactual cells are prepared for a later, optional inference run.
"""
    ...
    for key in sorted(expected):
        candidates = sorted(
            ...
            key=lambda row: _digest(
                row["packet"]["prompt_id"], row["packet_sha256"], "ablation_v1"
            ),
        )
...
def _variant_packet(
    ...
    elif variant == "persona_no_memory":
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if any(count != len(VARIANTS) for count in pair_counts.values()):
        errors.append("At least one ablation set does not contain all three generated variants")
    ...
    preflight = {
        ...
        "reference_policy": "observed full-persona/full-memory main-study response",
        "factorial_design": {
            "orientation_factor": ["full_persona", "neutral_orientation"],
            "memory_factor": ["full_memory", "no_memory"],
            "observed_reference_cell": "full_persona|full_memory",
            "generated_cells": [
