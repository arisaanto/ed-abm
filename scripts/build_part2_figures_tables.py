...
def _save_figure(fig: plt.Figure, path_without_suffix: Path) -> None:
    ...
    for suffix in (".pdf", ".svg"):
        fig.savefig(path_without_suffix.with_suffix(suffix), bbox_inches="tight")
...
def _vga_spatial_table(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    specifications = (
        ...
        ("Through vision (m)", "Through vision", 1),
...
def _paired_effect_table(effects: pd.DataFrame) -> pd.DataFrame:
    metric_specs = (
        ("F2F interactions per hour", 1.0, 2, "Δ F2F/hour [95% CI]"),
        ("Actionable visibility opportunities", 1.0, 1, "Δ actionable opportunities [95% CI]"),
        ("Staff–staff interaction share", 100.0, 1, "Δ staff–staff share, pp [95% CI]"),
        ("Patient-facing interaction share", 100.0, 1, "Δ patient-facing share, pp [95% CI]"),
        ("Corridor interaction share", 100.0, 1, "Δ corridor share, pp [95% CI]"),
    )
...
def _movement_effect_table(effects: pd.DataFrame) -> pd.DataFrame:
    metric_specs = (
        (
            "Total staff movement distance (km)",
            1,
            "Δ staff movement, km/run [95% CI]",
        ),
        ("Zone transitions", 1, "Δ zone transitions/run [95% CI]"),
        (
            "Station occupancy (staff-hours)",
            2,
            "Δ station occupancy, staff-hours/run [95% CI]",
        ),
    )
    rows: list[dict[str, str]] = []
    for scenario in SCENARIOS:
        for intervention in INTERVENTIONS:
            ...
            for metric, digits, label in metric_specs:
                ...
                if metric == "Total staff movement distance (km)":
                    ...
                    record["Δ staff movement (%)"] = f"{percent:+.1f}"
            rows.append(record)
    column_order = [
        "Scenario",
        "Contrast",
...
def _coordination_friction_table(runs: pd.DataFrame) -> pd.DataFrame:
    ...
    metric_specs = (
        ("Metres travelled per staff–staff contact", "Coordination cost (m/staff contact)", 1),
    ...
    for scenario in SCENARIOS:
        ...
        separation_pivot = scenario_runs.pivot(
            index="seed",
            columns="condition",
            values="Mean separation between interactions (m)",
        )
        segregation_pivot = scenario_runs.pivot(
            index="seed",
            columns="condition",
            values="Role–zone segregation (0–1)",
        )
        for condition in CONDITIONS:
            subset = scenario_runs[scenario_runs["condition"] == condition]
            record = {
            ...
            if condition == "baseline":
                record["Δ separation, m [95% CI]"] = "—"
                record["Δ segregation [95% CI]"] = "—"
                record["Δ footprint, pp [95% CI]"] = "—"
            else:
                separation_change = (
                    separation_pivot[condition] - separation_pivot["baseline"]
                ).dropna().to_numpy(dtype=float)
                separation_low, separation_high = _ci95(separation_change)
                record["Δ separation, m [95% CI]"] = _format_estimate(
                    float(np.mean(separation_change)), separation_low, separation_high, digits=2
                )
                segregation_change = (
                    segregation_pivot[condition] - segregation_pivot["baseline"]
                ).dropna().to_numpy(dtype=float)
                segregation_low, segregation_high = _ci95(segregation_change)
                record["Δ segregation [95% CI]"] = _format_estimate(
                    float(np.mean(segregation_change)), segregation_low, segregation_high, digits=3
                )
                footprint_change = (
...
def _load_sensitivity_source(sensitivity_dir: Path) -> pd.DataFrame:
    ...
    archive = PROJECT_DIR / "part2_parameter_sensitivity_n20_outputs.zip"
    if archive.exists():
...
def _plot_sensitivity(
    ...
    for row_index, scenario in enumerate(SCENARIOS):
        for column_index, factor in enumerate(FACTOR_ORDER):
            ...
            _despine(axis)
    axes[0, 0].text(
        0.02, 0.95, "Normal load", transform=axes[0, 0].transAxes,
        ha="left", va="top", fontsize=7.8, fontweight="normal", color=COLORS["ink"],
    )
    axes[1, 0].text(
        0.02, 0.95, "High load", transform=axes[1, 0].transAxes,
        ha="left", va="top", fontsize=7.8, fontweight="normal", color=COLORS["ink"],
    )
...
def _markdown_table(frame: pd.DataFrame, *, bold_columns: Sequence[str] = ()) -> str:
    ...
    for row in frame.itertuples(index=False, name=None):
        ...
        values = [
            f"**{value}**" if column in emphasized else value
...
def _latex_table(frame: pd.DataFrame, *, bold_columns: Sequence[str] = ()) -> str:
    ...
    for row in frame.itertuples(index=False, name=None):
        ...
        for column, value in zip(columns, row):
            ...
            values.append(r"\textbf{" + escaped + "}" if column in emphasized else escaped)
...
def _write_table(
    ...
    *,
    ...
    if tex_and_markdown:
        base_path.with_suffix(".md").write_text(_markdown_table(frame, bold_columns=bold_columns))
        base_path.with_suffix(".tex").write_text(_latex_table(frame, bold_columns=bold_columns))
...
def _write_vga_table(frame: pd.DataFrame, base_path: Path, metrics: pd.DataFrame) -> None:
    note = (
        ...
        "is the mean 360-degree ray-cast visible area. Through vision is the mean cumulative "
        "Euclidean length of direct visibility links per grid point. Visual integration is mean "
    ...
    _write_table(frame, base_path, bold_columns=("Metric",))
    point_counts = ", ".join(
...
def _write_effect_table(frame: pd.DataFrame, base_path: Path) -> None:
    ...
    _write_table(frame, base_path, bold_columns=("Δ F2F/hour [95% CI]",))
    markdown = base_path.with_suffix(".md")
    markdown.write_text(markdown.read_text() + f"\n*Note.* {note}\n")
    latex = base_path.with_suffix(".tex")
    latex.write_text(
...
def _write_ecology_table(frame: pd.DataFrame, base_path: Path) -> None:
    note = (
        ...
        "same-scenario baseline. Separation, segregation, and footprint change intervals use paired "
    ...
    _write_table(frame, base_path, bold_columns=("Condition",))
    markdown = base_path.with_suffix(".md")
    markdown.write_text(markdown.read_text() + f"\n*Note.* {note}\n")
    latex = base_path.with_suffix(".tex")
    latex.write_text(
...
def build(args: argparse.Namespace) -> None:
    ...
    _write_table(
        _movement_effect_table(effects),
        spatial_tables / "tableA_movement_effects",
        bold_columns=("Δ staff movement, km/run [95% CI]",),
    )
    _write_vga_table(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--canonical-dir",
        type=Path,
        default=PROJECT_DIR / "outputs" / "latest" / "ABM_results" / "part2_spatial_interventions_n100",
    )
    parser.add_argument(
        "--spatial-out",
        type=Path,
