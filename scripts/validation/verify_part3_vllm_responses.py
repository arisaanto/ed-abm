...
def _is_architecture_ablation(packets: list[dict[str, Any]]) -> bool:
    """Return whether every packet belongs to the matched architecture ablation."""
    ...
    return bool(
        packets
        and all(
            packet.get("ablation_not_main_result") is True
            and packet.get("ablation_variant") in variants
            and bool(packet.get("ablation_pair_id"))
...
def _appraisal_errors(
    ...
    if packet_type == "end_of_shift_survey_bundle":
        ...
        for row in rows:
            ...
            for field_name, public in (
                ("short_rationale", True),
                ...
                if issues:
                    errors.append(
                        f"{prompt_id}: {dimension} {field_name} failed prose "
        ...
        for row in rows:
            ...
            if (design_hypothesis is None) != (tradeoff is None) or (
                isinstance(design_hypothesis, str)
                and not design_hypothesis.strip()
            ) or (
                isinstance(tradeoff, str) and not tradeoff.strip()
            ):
                errors.append(
                    f"{prompt_id}: {question_id} has an incomplete design/tradeoff pair"
                )
            if question_id == "counterfactual_change":
                if design_hypothesis is None or tradeoff is None:
                    errors.append(
                        f"{prompt_id}: counterfactual_change lacks its required "
                        "design/tradeoff pair"
                    )
            elif design_hypothesis is not None or tradeoff is not None:
                errors.append(
                    f"{prompt_id}: {question_id} contains a redundant design/tradeoff pair"
                )
            prose_fields = {
                "answer": (row.get("answer"), True),
                "grounded_pattern": (row.get("grounded_pattern"), False),
                "persona_conditioned_interpretation": (
                    row.get("persona_conditioned_interpretation"),
                    False,
                ),
                "uncertainty_note": (row.get("uncertainty_note"), False),
            }
            for field_name in ("latent_need", "design_hypothesis", "tradeoff"):
                if row.get(field_name) is not None:
                    prose_fields[field_name] = (row.get(field_name), False)
            for field_name, (value, public) in prose_fields.items():
                ...
                if issues:
                    errors.append(
                        f"{prompt_id}: {question_id} {field_name} failed prose "
                        "quality checks: " + "; ".join(issues)
                    )
            hits = [
                category
...
def verify(
    ...
    *,
    ...
    semantic_retry_packet_count = 0
    semantic_retry_total = 0
    semantic_correction_exhausted_count = 0
    reference_matches = 0
    ...
    for result in responses:
        ...
        if packet is None:
            continue
        semantic_retry_count = int(result.get("semantic_retry_count", 0) or 0)
        semantic_retry_total += semantic_retry_count
        semantic_retry_packet_count += int(semantic_retry_count > 0)
        generation_error = result.get("generation_error")
        if generation_error:
            semantic_correction_exhausted_count += 1
            errors.append(
                f"{prompt_id}: generation exhausted semantic correction attempts: "
                f"{generation_error}"
            )
    ...
    architecture_ablation_checked = _is_architecture_ablation(packets)
    scientific_design_checked = bool(
        reject_fixtures
        and packets
        and not architecture_ablation_checked
        and all(
            packet.get("fixture_only_not_scientific_data") is False
            and packet.get("packet_type") == "in_simulation_decision"
            for packet in packets
    ...
    return {
        ...
        "architecture_ablation_checked": architecture_ablation_checked,
        ...
        "semantic_retry_packet_count": semantic_retry_packet_count,
        "semantic_retry_total": semantic_retry_total,
        "semantic_retry_packet_rate": (
            semantic_retry_packet_count / len(responses) if responses else None
        ),
        "semantic_correction_exhausted_count": (
            semantic_correction_exhausted_count
        ),
