...
*Note.* VGA-style approximations on a 1.0 m regular grid over the largest connected walkable component. Connectivity is the mean count of directly visible grid points. Isovist area is the mean 360-degree ray-cast visible area. Visual integration is mean normalized closeness centrality on the visibility graph. Visual intelligibility is the Pearson correlation between local connectivity and global visual integration across grid points. Condition-specific visibility walls are used throughout; these are not formal DepthmapX outputs. Sample points: Baseline n=250, COCPIT only n=250, NURSTA only n=253, Both n=253.
